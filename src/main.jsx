...
import { 
  ...
  TrendingDown, Sparkles, Loader2, Utensils, Dumbbell, BarChart3, LayoutDashboard
} from 'lucide-react';
                  ...
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                    ...
                    <div className="flex bg-black/20 p-1.5 rounded-2xl gap-2">
                      <select value={planMood} onChange={(e) => setPlanMood(e.target.value)} className="bg-transparent text-white border-none px-4 py-2 text-xs font-black">
                        <option value="motivated">Energetic</option>
              ...
              <div className="space-y-8">
                <div className="bg-gradient-to-br from-orange-500 to-rose-600 rounded-[2.5rem] p-8 text-white shadow-2xl">
                  ...
                  <form onSubmit={handleAddWeight} className="space-y-6">
                    ...
                    <button className="w-full bg-white text-orange-600 font-black py-5 rounded-3xl shadow-xl">Submit Record</button>
                  </form>
                </div>
                <div className="bg-[#1e293b] rounded-[2.5rem] p-8 border border-white/5 shadow-xl">
...
const StatCardV2 = ({ icon, label, value, unit, sub, subColor }) => (
  <div className="bg-[#1e293b] p-8 rounded-[2.5rem] border border-white/5 shadow-2xl">
    <div className="bg-slate-900/50 p-4 rounded-2xl w-fit mb-6">{icon}</div>
...
const PlanCard = ({ icon, color, title, text }) => (
  <div className="bg-white/5 p-6 rounded-3xl border border-white/5">
    <div className={`${color} w-10 h-10 rounded-xl flex items-center justify-center mb-4`}>{icon}</div>
...
const MetricRow = ({ label, value, unit, color }) => (
  <div className="flex justify-between items-center p-6 bg-slate-50 rounded-[2rem]">
    <span className="text-xs text-slate-400 font-black uppercase">{label}</span>
